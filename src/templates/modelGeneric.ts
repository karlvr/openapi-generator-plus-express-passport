import { CodegenObjectSchema, CodegenGeneratorContext } from '@openapi-generator-plus/types'
...
const CONVERT_TO_LITERAL_TYPE = 'convert-to-literal-type'
...
export function modelGeneric(generatorContext: CodegenGeneratorContext, schema: CodegenObjectSchema): string {
	...
	const literalType = schema.vendorExtensions?.[CONVERT_TO_LITERAL_TYPE]
	if (literalType !== undefined) {
		return ts`
export type ${name} = ${String(literalType)}
