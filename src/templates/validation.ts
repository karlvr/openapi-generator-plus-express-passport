...
export function validation(generatorContext: CodegenGeneratorContext, ctx: DocumentContext): string {
	const native = ctx.dateApproach === DateApproach.Native
	const dateTimeType = native ? 'Date' : 'string'
...
export function dateTimeFromRequest(name: string, value: any): ${dateTimeType} {
	if (typeof value !== 'string') {
		throw \`Invalid type for \${name}: expected string got \${typeof value}\`
	}
	...
	${native ? 'return new Date(value)' : 'return value'}
}

export function parseDateTime(name: string, value: any): ${dateTimeType} {
	return dateTimeFromRequest(name, value)
}

export function dateTimeToResponse(name: string, value: ${dateTimeType}): string {
	${native ? ts`
if (typeof value !== 'object' || typeof value.toISOString !== 'function') {
	throw \`Invalid type for \${name}: expected Date got \${typeof value}\`
}
return value.toISOString()` : 'return dateTimeFromRequest(name, value)'}
}
...
${nestedValidation(generatorContext, ctx)}
`
}
